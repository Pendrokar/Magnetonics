...
local function GetStartUnit()
	local sidedata = Spring.GetSideData()
	return sidedata[1].startUnit
end

local function GetStartExtraUnit()
	local sidedata = Spring.GetSideData()
	return sidedata[1].startExtraUnit
end
...
local function SpawnStartUnit(teamID)
	...
	if (startUnit and startUnit ~= "") then
		...
		Spring.CreateUnit("lightfactory", x, y, z, facing, teamID)
	end

	-- set start resources, either from mod options or custom team keys
	local teamOptions = select(7, Spring.GetTeamInfo(teamID))
	local m = teamOptions.startmetal  or modOptions.startmetal  or 100000
	local e = teamOptions.startenergy or modOptions.startenergy or 100000
